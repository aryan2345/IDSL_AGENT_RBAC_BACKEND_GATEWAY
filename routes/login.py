...
async def login(user: UserLogin):
    ...
    user_data = db.fetch_one(
        "SELECT * FROM users WHERE username = %s AND role != 'deactivated'",
        (user.username,)
    )

    if not user_data or user_data['password_hash'] != hash_password(user.password):
        # Lookup user_id if username exists to comply with NOT NULL
    ...
    log_audit(user_data['user_id'], "/login", 200, "Login successful")
