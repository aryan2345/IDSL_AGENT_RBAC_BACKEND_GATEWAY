...
async def create_conversation(request: ChatRequest, current_user: dict = Depends(verify_token)):
    ...
    # Insert new chat in PostgreSQL
    try:
        db.execute_query(
            "INSERT INTO chats (chat_id, user_id, chat_name, chat_content) VALUES (%s, %s, %s, %s)",
...
async def get_conversation_history(request: ChatRequest, current_user: dict = Depends(verify_token)):
    try:
        # Optional: Check if chat belongs to user in SQL
        check = db.fetch_one("SELECT 1 FROM chats WHERE chat_id = %s AND user_id = %s", (request.chat_id, current_user["user_id"]))
        if not check:
...
async def update_conversation(request: UpdateChatRequest, current_user: dict = Depends(verify_token)):
    try:
        # Optional: Check if chat belongs to user
        check = db.fetch_one("SELECT 1 FROM chats WHERE chat_id = %s AND user_id = %s", (request.chat_id, current_user["user_id"]))
        if not check:
